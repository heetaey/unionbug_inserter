...
class UnionBugApp:
    def __init__(self, root):
        ...
        self.black_bug = os.path.join(bundle_dir, "UnionBug - Small Black.pdf")
        self.white_bug = os.path.join(bundle_dir, "UnionBug - Small White.pdf")
        ...
        self.scale_x = 1
        self.scale_y = 1
        self.SAFE_MARGIN_PT = 20
        self.bug_width_in = 0.3  # Default bug width in inches
    ...
    def on_checkbox_change(self, event=None):
        # Get which page we're currently previewing
    ...
    def update_page_selection(self, event=None):
        ...
        self.trimbox = (
            page.trimbox if page.trimbox != fitz.Rect(0, 0, 0, 0)
            else page.cropbox if page.cropbox != fitz.Rect(0, 0, 0, 0)
            else page.rect
        )
        tw, th = round(self.trimbox.width / 72, 2), round(self.trimbox.height / 72, 2)
    ...
    def render_preview(self):
        ...
        print("trimbox:", self.trimbox)
        print("rect:", page.rect)
        print("scale_x:", self.scale_x, "scale_y:", self.scale_y)

        # 1. Draw trim area (RED)
        trim = self.trimbox
        ...
        # 2. Draw bleed area (ORANGE) = full page.rect
        bleed = page.rect
        ...
        # 3. Draw safe zone (GREEN) = inset from trim
        SAFE_MARGIN_PT = 9  # 0.125"
        ...
        # ✅ Only show preview if page is selected AND not disabled
        selected_indices = self.page_listbox.curselection()
        if (
            self.selected_page in selected_indices and
            self.selected_page not in self.no_placement and
            (self.selected_page in self.click_coords or len(self.click_coords) > 0)
        ):
            print(f"Rendering bug preview for page {self.selected_page} (fallback: {self.click_coords})")
    ...
    def render_bug_preview(self):
        ...
        if coords:
            ...
            # Fallback from the first page that was clicked
        ...
        self.chosen_bug = self.choose_bug_by_contrast(x, y)

        bug_doc = fitz.open(self.chosen_bug)
        bug_page = bug_doc[0]
        bug_rect = bug_page.rect

        # Target bug width in points, then scale
        target_width_pt = self.bug_width_in * 72
        scale = target_width_pt / bug_rect.width

        # Render the bug image at actual size (points) and match the preview scale
        ...
        # Now scale the bug preview image to match the page scaling
    ...
    def clear_placement(self):
        try:
            ...
            self.click_coords = {}  # page index -> (x, y)
    ...
    def choose_bug_by_contrast(self, x, y):
        page = self.doc[self.selected_page]
        ...
        brightness = sum(box.convert("L").getdata()) / 100
        return self.white_bug if brightness < 128 else self.black_bug
    ...
    def place_and_save(self):
        try:
            ...
            target_width = self.bug_width_in * 72
            ...
            for idx in selected_indices:
                ...
                # Use this page's trimbox to determine placement
                trimbox = (
                    page.trimbox if page.trimbox != fitz.Rect(0, 0, 0, 0)
                    else page.cropbox if page.cropbox != fitz.Rect(0, 0, 0, 0)
                    else page.rect
                ...
                if page_num in self.click_coords:
                    x, y = self.click_coords[page_num]
                elif self.click_coords:
                    first_page = next(iter(self.click_coords))
                    x, y = self.click_coords[first_page]
                else:
                ...
                if x is not None and y is not None:
                    # Clamp union bug within trimbox
                    ...
                    x = bleed_x + trimbox.width - target_width - self.SAFE_MARGIN_PT
                    y = bleed_y + trimbox.height - target_height - self.SAFE_MARGIN_PT
            ...
            self.doc.save(output_path, incremental=False)
    ...
    def on_page_listbox_change(self, event=None):
        ...
        if selected_indices:
            ...
            self.update_page_selection()
            self.render_preview()  # ✅ force re-render to show fallback
    ...
    def clear_current_page_placement(self):
        ...
        # If this page is the only one placed, warn user it's the fallback
